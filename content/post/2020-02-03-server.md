...
备份命令为：
```bash
docker export -o vaultwarden-xxx.tar vaultwarden # 备份容器
docker run --rm --volumes-from vaultwarden -v $(pwd):/backup vaultwarden/server tar cvf /backup/bw-data.tar /data # 备份volume
```
